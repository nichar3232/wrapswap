...
if not os.getenv('ETHERSCAN_API_KEY'):
    sys.exit('Contracts deployed; Basescan verification blocked: ETHERSCAN_API_KEY is unset.')
p=pathlib.Path('broadcast/Deploy.s.sol/84532/run-latest.json')
if not p.exists(): p=pathlib.Path('contracts/broadcast/Deploy.s.sol/84532/run-latest.json')
...
for t in d['transactions']:
    ...
    r=subprocess.run(['forge','verify-contract',addr,name,'--chain','84532','--watch','--constructor-args',encoded,'--etherscan-api-key',os.environ['ETHERSCAN_API_KEY']])
