...
Conventions in this file: upstream code is linked by pinned permalink; our code is linked by file and line at tag `v1-ethglobal-tokyo`. Design decisions are cited as "DECISIONS.md: <entry>". Each item has three parts: **What happened** (what we actually hit), **Where** (the exact API or file), and **Suggested fix**.
...
**What happened.** Hook inventory is ERC-6909 claims owned by ParityHook in the PoolManager. Keepers deposit it with [`ParityHook.depositInventory` (ParityHook.sol#L126)](contracts/src/ParityHook.sol#L126) and withdraw it with [`ParityHook.withdrawInventory` (ParityHook.sol#L134)](contracts/src/ParityHook.sol#L134) (DECISIONS.md: "Inventory is ERC-6909 claims owned by ParityHook in the PoolManager…"). A fill mints input claims to the hook and burns output claims inside `beforeSwap`, with no ERC-20 transfer in the callback. Fees retained by the hook land in the **same** claim balance as inventory: ERC-6909 has one balance per (owner, id). An earlier design tracked `feesAccrued` separately and excluded it from spendable inventory, which meant a second ledger that every fill, withdrawal and skew calculation had to respect. The final design removes that ledger: the whole fee stays in inventory as LP revenue, so claim balance = inventory by construction, and fee revenue is reported from the `Converted` event's share-denominated `baseFee`/`skewFee` fields rather than read from a balance ([`ParityHook.beforeSwap` (ParityHook.sol#L270-L272)](contracts/src/ParityHook.sol#L270-L272)). Mixed decimals still bite: raw amounts in a 6-decimal and an 18-decimal token can't be added, so all fee and skew math is in canonical share units (DECISIONS.md: "Express hook fee PnL in share units").
...
**Suggested fix.** Publish a worked example of a "claims-backed hook inventory". It should cover the choice between sub-accounting fees inside one 6909 balance and leaving them in inventory, the invariant to test for each, and a settlement ledger with four columns: caller delta, hook delta, ERC-20 balance of the PoolManager, and claim balance. Show it for one exact-in fill and one exact-out fill.
