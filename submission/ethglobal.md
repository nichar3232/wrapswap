...
Uniswap v4 hook: share-for-share conversion between tokenized-stock wrappers, no cash leg.

## Description

The same Apple share trades on-chain through different issuers, such as Coinbase's tokenized AAPL (which lives on Base) and Backed's xStocks AAPLx. Each wrapper has its own liquidity, and neither issuer redeems the other's token, so switching issuers today means selling for cash and buying back.

Unison converts share for share, with no cash leg. It prices both wrappers in canonical shares (via each issuer's multiplier) and runs on Unichain Sepolia (chain 1301) and Sui testnet for AAPL, NVDA and TSLA. The app has four tabs, Portfolio · Move · Send · Liquidity, where Move holds Convert and Dark Cross. Three products:

- **Convert.** A Uniswap v4 hook, ParityHook, runs directly on a pool of the two issuer tokens (one hook, one pool per asset). It fills the whole swap from its own ERC-6909 inventory at the adapters' share ratio (1 mcbAAPL = 1.0125 shares = 1.0125 mAAPLx in the demo). If inventory can't cover a swap, it executes against the same pool's concentrated liquidity, and a 50 bps peg guard reverts any swap that would move the price further than that from parity.
- **Dark Cross.** Orders commit sealed, reveal, and cross in a batch at the oracle mid (at most 30 minutes old), paying 1 bp of each side's crossed amount to the protocol fee recipient. The uncrossed residual fills from ParityHook inventory inside the same `unlock`; any part inventory can't fill is refunded, and unrevealed commits are forfeited to the protocol.
- **Send (Unison Pay).** Pay someone in shares confidentially: deposit a wrapper into the ShareVault on Unichain, send a Seal-encrypted payment on Sui (Walrus for storage), and the recipient withdraws into whichever issuer's wrapper they use.

Users only ever hold real issuer securities; shares are internal accounting. Swaps are gated to non-US users through the Coinbase Verified Country EAS attestation. On testnet, an owner-set `demoMode` bypasses the gate so anyone can try it, and that toggle is public on-chain. A Unison MCP server gives agents the same read and execute surface as the app.

The Convert fee prices only inventory risk: 2 bps base (owner-settable) + a skew fee of min(15 bps × |post-trade skew|, 50 bps) charged only to trades that increase inventory imbalance; the whole fee stays in the hook's inventory (100% to the LP), the protocol takes nothing on Convert. There is no market-hours component. At time of writing (AAPL |skew| 0.188), 100 mcbAAPL → mAAPLx on Unichain Sepolia costs the 2.00 bps base and the reverse costs 4.97 bps (2.00 + 2.97 skew).
...
- **ParityHook** uses the `beforeInitialize | beforeSwap | afterSwap | beforeSwapReturnDelta` permissions, mined into its address through the CREATE2 proxy.
  ...
  - Sources: [`ParityHook.beforeSwap` (ParityHook.sol#L245)](../contracts/src/ParityHook.sol#L245), [`ParityHook.afterSwap` (ParityHook.sol#L297)](../contracts/src/ParityHook.sol#L297), [`ParityHook.quote` (ParityHook.sol#L175)](../contracts/src/ParityHook.sol#L175); DECISIONS.md: "Rounding favours the hook".
- **DarkCrossHook** has no pool attached. It escrows the token being sold, verifies commit hashes on reveal, and settles permissionlessly at an oracle mid, which must be no older than 1800 s (`ORACLE_MAX_AGE`). Crossed volume pays `CROSS_FEE_PIPS = 100` (1 bp) per side to the protocol fee recipient. Residuals fill from ParityHook inventory within one `PoolManager.unlock` at base + skew, the unfilled part is refunded, and a failure on one trader's residual is isolated from the rest of the batch. Sources: [`DarkCrossHook.settle` (DarkCrossHook.sol#L262)](../contracts/src/DarkCrossHook.sol#L262); DECISIONS.md: "Isolate unfillable residual self-calls and unlock their collateral".
...
- **Send.** ShareVault on Unichain Sepolia holds deposited wrappers; the `unison_pay` Move package on Sui testnet carries Seal-encrypted payments (Walrus storage), and a Sui keeper settles withdrawals back on Unichain into either issuer's wrapper. Solvency invariant: Sui `total_shares` ≤ shares held by the ShareVault.
- **Off-chain.** A TypeScript API and indexer on Postgres 16. The indexer handles reorgs with block-hash cascades (DECISIONS.md: "Indexer reorg safety comes from cascading deletes"). A crank pushes the mock oracle mid, runs `checkPeg` and settles batches. A Unison MCP server (stdio and Streamable HTTP) exposes read and execute tools to agents ([packages/mcp/README.md](../packages/mcp/README.md)). The web app uses React, Vite and viem, and a shared `@wrapswap/types` package re-verifies the canonical demo arithmetic on every build.
- **Tricky parts** (detailed in FEEDBACK.md):
  ...
  - Coinbase's B20 token not executing on generic Anvil, so the demo uses testnet mocks with issuer-faithful decimals and multipliers.

## Tech stack

Solidity 0.8.26, Foundry, Uniswap v4-core / v4-periphery (PoolManager, ERC-6909 claims, dynamic fees, PositionManager, Permit2, V4Quoter), Ethereum Attestation Service (Coinbase Verified Country), Unichain Sepolia, Sui (Move, Seal, Walrus), MCP, TypeScript, Node.js, viem, Postgres 16, React + Vite, Playwright.
