...
it("HTTP requests survive macOS socket QoS failures", async () => {
  ...
  await new Promise<void>((resolve) =>
    server.listen(Number(process.env.WEB_PORT ?? 13004), "127.0.0.1", resolve),
  );
  const proto = Socket.prototype as any;
  const original = proto.setTypeOfService;
  proto.setTypeOfService = function () {
  ...
  try {
    ...
    const replies = await Promise.all(
      Array.from({ length: 10 }, async () =>
        (await fetch(`http://127.0.0.1:${address.port}`)).text(),
      ),
    );
    expect(replies).toEqual(Array(10).fill("ok"));
  } finally {
    proto.setTypeOfService = original;
