...
interface IDeployDark {
    function configurePool(PoolKey calldata) external;
}

interface IDeployPosition {
    function initializePool(PoolKey calldata, uint160) external payable returns (int24);
}

interface IDeployFeed {
...
contract Deploy is Script {
    ...
    function run() external {
        n = Addresses.forChain(block.chainid);
        ...
        address adapter2 =
            _deploy("StaticAdapter.sol:StaticAdapter", abi.encode(issuer2, "Mock Apple xStock", uint256(1e18), owner));
        IDeployRegistry(registry).add(adapter1);
        IDeployRegistry(registry).add(adapter2);
        ...
        if (block.chainid == 84532) {
            n.usdc = _deploy("MockIssuerToken.sol:MockIssuerToken", abi.encode("Mock USD Coin", "USDC", uint8(6)));
            n.oracle = address(new MockOracle(33972e6));
        }
        ...
        (, int256 px,,,) = IDeployFeed(n.oracle).latestRoundData();
        require(px > 0, "invalid oracle");
        ...
        console2.log("uAAPL", vault);
        console2.log("ParityHook", parity);
        console2.log("DarkCrossHook", dark);
    }
    ...
    function _deploy(string memory artifact, bytes memory args) internal returns (address addr) {
        ...
        assembly ("memory-safe") { addr := create(0, add(code, 32), mload(code)) }
        require(addr != address(0), "deployment failed");
    }

    function _hook(string memory artifact, bytes memory args, uint160 flags) internal returns (address addr) {
        bytes memory code = vm.getCode(artifact);
        bytes32 salt;
        (addr, salt) = HookMiner.find(CREATE2_PROXY, flags, code, args);
        bytes memory init = abi.encodePacked(salt, code, args);
        (bool ok,) = CREATE2_PROXY.call(init);
        require(ok && addr.code.length > 0, "CREATE2 failed");
    }

    function _key(address a, address b, uint24 fee, address hook) internal pure returns (PoolKey memory) {
        return PoolKey(
            Currency.wrap(a < b ? a : b),
            Currency.wrap(a < b ? b : a),
            fee,
            fee == 500 ? int24(1) : int24(60),
            IHooks(hook)
        );
    }

    function _sqrtFor(address a, address b, uint256 amountA, uint256 amountB) internal pure returns (uint160) {
        uint256 ratio = a < b ? amountB * 1e18 / amountA : amountA * 1e18 / amountB;
        return uint160(Math.sqrt(ratio) * (1 << 96) / 1e9);
    }
    ...
    function _pool(PoolKey memory k, string memory kind) internal view returns (string memory) {
        return string.concat(
            '{"id":"',
            vm.toString(PoolId.unwrap(k.toId())),
            '","kind":"',
            kind,
            '","key":{"currency0":"',
            ...
            '","fee":',
            vm.toString(uint256(k.fee)),
            ',"tickSpacing":',
            vm.toString(int256(k.tickSpacing)),
            ',"hooks":"',
    ...
    function _manifest(PoolKey memory p1, PoolKey memory p2, PoolKey memory lit) internal {
        string memory object = "contracts";
        vm.serializeAddress(object, "poolManager", n.poolManager);
        vm.serializeAddress(object, "positionManager", n.positionManager);
        vm.serializeAddress(object, "stateView", n.stateView);
        vm.serializeAddress(object, "quoter", n.quoter);
        vm.serializeAddress(object, "universalRouter", n.universalRouter);
        ...
        vm.serializeAddress(object, "darkCrossHook", dark);
        vm.serializeAddress(object, "oracle", n.oracle);
        string memory contracts_ = vm.serializeAddress(object, "eas", n.eas);
        string memory json = string.concat(
            ...
            '","mockB20":',
            mockB20 ? "true" : "false",
            ',"demoMode":true,"deployer":"',
            vm.toString(owner),
            '","contracts":',
            contracts_,
            ',"tokens":{"uAAPL":',
            _token(vault, "uAAPL", 18),
            ',"issuer1":',
            _token(issuer1, mockB20 ? "mAAPLc" : "AAPLc", 8),
            ',"issuer2":',
            _token(issuer2, "mAAPLx", 18),
            ',"USDC":',
            _token(n.usdc, "USDC", 6),
            '},"pools":[',
            _pool(p1, "parity"),
            ",",
            _pool(p2, "parity"),
