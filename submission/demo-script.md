...
| Claim | Source |
|---|---|
| Hook fills from inventory inside beforeSwap at the share ratio | {{LINE:ParityHook.beforeSwap(address,PoolKey,SwapParams,bytes)}}; DECISIONS.md "Fills are all-or-nothing from inventory via beforeSwapReturnDelta" |
| Fee = 2 bps base + skew + 10 bps closed, cap 25 bps | {{LINE:ParityHook.feeBreakdown(PoolKey)}}; DECISIONS.md "Fee is expressed in pips: min(200 + ceil(1300·\|skew\|) + (NYSE closed ? 1000 : 0), 2500)" |
| Inventory is ERC-6909 claims in the PoolManager | {{LINE:ParityHook.depositInventory(Currency,uint256)}}; DECISIONS.md "Inventory is ERC-6909 claims owned by ParityHook in the PoolManager" |
| Fall-through to the same pool, 50 bps peg guard | {{LINE:ParityHook.afterSwap(address,PoolKey,SwapParams,BalanceDelta,bytes)}}; DECISIONS.md "afterSwap reverts when the post-swap price is more than 50 bps from adapter parity" |
| Commit-reveal; settle at oracle mid; 5 bps per side | {{LINE:DarkCrossHook.commit(bytes32,address,uint256,bytes32)}}, {{LINE:DarkCrossHook.reveal(bool,uint256,uint256,bool,bytes32)}}; DECISIONS.md "crossing charges 5 bps per side to treasury" |
| Residual swapped into the ParityHook pool in the same unlock | {{LINE:DarkCrossHook.settle(uint256)}}; DECISIONS.md "routes residuals into the ParityHook pool inside the same unlock" |
| Non-US gate via Coinbase Verified Country EAS; demoMode on testnet | {{LINE:IEligibility.check(address,bytes32)}}, {{LINE:IEligibility.setDemoMode(bool)}}; DECISIONS.md "IEligibility has an EAS implementation (Coinbase Verified Country, restricted country \"US\")… demoMode is owner-set and emits DemoModeSet" |
| NYSE closed read from the chain clock | {{LINE:NyseCalendar.isOpen(uint256)}}; DECISIONS.md "Market-hours logic everywhere reads the latest block timestamp" |
