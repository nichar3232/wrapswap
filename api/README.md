...
| --- | --- |
| `NETWORK` | Required: `anvil` or `unichain-sepolia`; selects the validated deployment JSON. |
...
Routes: `/health`, `/deployment`, `/pool`, `/inventory`, `/inventory/changes`, `/fees`, `/quote`, `/route`, `/nyse`, `/batches/current`, `/batches`, `/batches/:batchId`, `/orders/:address`, `/fills`, `/eligibility/:address`. Every request query and response uses the shared validators. `/status` additionally proxies crank health using `CrankStatusResponse`. `/health` retains the exact frozen schema and always returns HTTP 200, with failures in its fields.
