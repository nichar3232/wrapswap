...
- 2026-09-26: Fee is expressed in pips: min(200 + ceil(1300·|skew|) + (NYSE closed and the trade increases |skew| ? ceil(1500·|post-trade skew|) : 0), 2500), skew measured in canonical shares on pre-swap inventory (the off-hours term at post-trade skew; see the 2026-09-26 off-hours entry below): pips are the v4 LP-fee unit, allow the 13·|skew| term without truncation, and pre-swap skew makes quotes exact.
...
- 2026-09-26 (off-hours fee): The flat 10 bps closed-market premium is replaced by 15 bps · |post-trade skew|, charged only to trades that increase |skew| (skew-reducing trades pay 0). A same-share swap carries no underlying price risk; off-hours the only risk is rebalancing lag (issuers cannot mint/redeem until the open), which grows with skew. Post-trade skew moves the trade's canonical shares one-for-one, sized fee-independently (exact input: input shares; exact output: net output shares) so the fee is not circular. `feeBreakdown(key)` reports the marginal skew-increasing premium ceil(1500·|skew|). Base, skew, cap, peg guard and fee rounding (feeOnGross/grossForNet) are unchanged. Unichain Sepolia redeployed only ParityHook, the AAPL pool and DarkCrossHook (UpgradeFees.s.sol).
