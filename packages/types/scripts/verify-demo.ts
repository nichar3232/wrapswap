...
const variants = [DEMO.variants.anvil, DEMO.variants["base-sepolia"], DEMO.variants["unichain-sepolia"]];
let grossOut1 = 0n;
...
for (const n of ["base-sepolia", "unichain-sepolia"] as const) {
  const nextOpen = new Date((DEMO.variants[n].nextOpen - 4 * 3600) * 1000);
  eq(`${n} next open is Monday 09:30 EDT`, `${nextOpen.getUTCDay()} ${nextOpen.getUTCHours()}:${nextOpen.getUTCMinutes()}`, "1 9:30");
}
...
console.log("verify-demo: §10 ANVIL, BASE-SEPOLIA and UNICHAIN-SEPOLIA variants are arithmetically consistent");
