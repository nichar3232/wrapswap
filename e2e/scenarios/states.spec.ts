...
import { api,webURL,deployment,account,write,abis,DEMO } from '../support.js';

// Playwright's default runs cases in one file sequentially, without skipping later failures.
test.describe.configure({mode:'default'});
for(const variant of ['FALL-THROUGH','BLOCKED-PEG','BLOCKED-ELIGIBILITY']) {
  test(variant,async({page})=>{
    ...
    expect(route.route).toBe(variant);
