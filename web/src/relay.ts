...
const action = <T>(name: "convert" | "convert-all" | "dark-commit" | "send", body: unknown) => call<T>("POST", `/${name}`, body);
...
/** Every other wrapper balance of every asset into the `to` issuer (default xStocks), one swap per asset. */
