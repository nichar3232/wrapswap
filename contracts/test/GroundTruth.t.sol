...
interface IB20Probe {function decimals() external view returns(uint8); function multiplier() external view returns(uint256); function balanceOf(address) external view returns(uint256); function transfer(address,uint256) external returns(bool);}
...
contract GroundTruthTest is Test {
 ...
 function testGroundTruth() public {
  ...
  if(!ok || data.length==0){
   ...
   vm.prank(holder); assertTrue(IB20Probe(AAPL).transfer(recipient,1e8));
