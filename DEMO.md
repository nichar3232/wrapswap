...
|---|---|---|---|---|---|
| Unichain Sepolia (live, primary) | 13010 | 18010 | 18110 | 15410 (container `wrapswap-sepolia-pg`) | `scripts/dev/sepolia-up` |
...
## 1. Unichain Sepolia stack (primary)
...
- Secrets come from `~/wrapswap-run/env/lane6.env` (`DEMO_MNEMONIC`, `CRANK_PRIVATE_KEY`, `UNICHAIN_SEPOLIA_RPC_URL`). Override the path with `SEPOLIA_ENV=...`.
- Settings: `NETWORK=unichain-sepolia`, `USE_MOCKS=false`, `deployments/unichain-sepolia.json`, RPC `sepolia.unichain.org`, and the crank signs as `0xa9c1…30cB`.
...
- The crank's `lastError` sometimes shows `LimitExceededRpcError` from the rate-limited public RPC. It is a transient throttle, and the crank clears it on the next block.
...
pool already holds one live swap (below), so the Sepolia skew figures differ from the §10 UNICHAIN-SEPOLIA variant.
...
- Proof of an executed Convert through WrapSwapRouter: tx [0xf4de19cb…1de90](https://sepolia.uniscan.xyz/tx/0xf4de19cb48bb7ea8a2e729c87b99b0cbd1be51a56f6e586d58195c184cf1de90). It converted 100 mcbAAPL into 101.10227625 mAAPLx at 14.59 bps and is indexed as a PARITY fill.
...
- Sepolia settled batch 16, settle tx [0x7f640d9a…6b9](https://sepolia.uniscan.xyz/tx/0x7f640d9a4552de2a87bded2e5f819b26dbd5178019ee627c33cf93e43fbaa6b9). It produced two DARK-CROSS fills and one DARK-RESIDUAL fill (10 mcbAAPL → 10.110217500 mAAPLx through ParityHook), all under one settlement tx.
- On stage, show this settled batch and its tx; don't try to run a fresh one. Re-running `scripts/dev/seed` on Sepolia is a no-op by design, because the `logs/integration/seed-unichain-sepolia` marker blocks duplicate inventory. The live commit → reveal → settle sequence is shown in the anvil backup, where phases are 20 blocks.
- Anvil backup: batch 2 SETTLED. Crossed 50 mcbAAPL ↔ 50.625 mAAPLx; residual 10 mcbAAPL → 10.120474125 mAAPLx at 4.47 bps.

### 4.4 Uniscan proof

- ParityHook (verified): https://sepolia.uniscan.xyz/address/0x708AeC5CD2C504A8fB40615aC797C52BB2EB20c8#code (flags `0x20c8`)
