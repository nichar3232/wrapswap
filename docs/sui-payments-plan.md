...
| Issuer conversion and share custody | Base | `CanonicalStock`, `IssuerRegistry`, `ParityHook`, v4 pools, oracle, `NyseCalendar` |
...
Consequence worth stating plainly in any submission: **shares never leave Base.** Sui holds credits against Base-custodied uAAPL. This is why no share bridge is required, and it is also the source of the custody assumption in §3.

## 3. Trust model and custody

1. `ShareVault` on Base custodies uAAPL. Sui `Pool` credits are claims against that balance.
2. A single keeper attests deposits (Base → Sui credit) and executes withdrawals (Sui credit → Base issuer token). This is the same trust model already documented for the deferred intent bridge: **optimistic, single relayer.** Nautilus is the upgrade path (§14).
...
- `total_shares: u128` — the sum of all credits, which must equal Base vault holdings.
...
- `credit_deposit(pool, new_root, new_total, base_receipt, auth, ctx)` — increases `total_shares` against an attested Base deposit.
- `debit_withdrawal(pool, new_root, new_total, base_receipt, auth, ctx)` — decreases it against an attested Base settlement.
...
- Seal's documentation cautions against policies that depend on frequently changing on-chain state, because propagation across full nodes takes time. **Batch windows are therefore minutes, not blocks** — 10 minutes is the proposed default, unlike the Base-side 20-block DarkCross batch. Do not tighten this to seconds.
...
`settleWithdrawals` performs one `poolManager.unlock()` and, inside `unlockCallback`, converts uAAPL to each recipient's target issuer token at parity through the ParityHook pool, then transfers out. This is the leg that keeps Uniswap on the critical path: every exit is a v4 swap filled from hook inventory via `beforeSwapReturnDelta`, degrading to the existing `ZERO_DELTA` fall-through and 50 bps peg guard when inventory is short.
...
**Payment (Sui only, no Base transaction).** Payer encrypts `{payee, shares, memo}` under Seal, uploads to Walrus, calls `submit` with the commitment and blob id → window closes → `seal_approve_batch` now permits the operator → keeper decrypts, checks each payer's leaf for sufficient balance, computes the new leaf set and root → `apply_batch` with `new_total == total_shares`. Recipient sees the credit immediately after the batch applies, and can decrypt the memo.

**Withdraw to a brokerage.** User submits a withdrawal instruction naming their target issuer token → keeper batches the window's withdrawals → `settleWithdrawals` on Base converts uAAPL to each target wrapper at parity and transfers out → `debit_withdrawal` lowers `total_shares` and republishes the root.
...
/pay/reserves        -> {suiTotalShares,baseUHeld,invariant:boolean,checkedBlock}
```
...
pay_withdrawals(commitment TEXT PRIMARY KEY, recipient TEXT, target_token TEXT,
                shares NUMERIC, base_tx TEXT, log_index INTEGER, skipped_reason TEXT)
pay_reserves(checked_block NUMERIC PRIMARY KEY, sui_total NUMERIC, base_held NUMERIC,
             invariant BOOLEAN)
```
...
1. **Custody.** The Base vault holds the shares; Sui credits are claims. This is a custodial design.
...
3. **Not anonymous.** Internal transfers are unlinkable to the public; deposits and withdrawals are visible on Base, and the anonymity set at the boundary is the withdrawal batch size.
...
7. **Testnet only.** Walrus has no public mainnet publisher, and Base Sepolia deployment remains blocked by zero deployer ETH.
...
1. `ShareVault.sol`: deposit, batched `settleWithdrawals` through the parity pool, per-withdrawal isolation, gross-up, hook-output rounding. Testable on the existing local fork with the existing suite.
2. Move package: `Pool`, `Batch`, `submit`, `apply_batch`, both `seal_approve` clauses. Sui testnet is free, so this is not blocked by the Sepolia funding gap.
3. Keeper: a Sui client in `services/crank` alongside the Base one — watch windows, decrypt, verify balances, compute the root, apply, attest deposits and withdrawals.
...
Minimum end-to-end demo: deposit an issuer wrapper on Base, pay a second user privately on Sui with nothing but a root change, then have that user withdraw into a **different** issuer's wrapper through the parity pool. That single path demonstrates the whole thesis, and it must exist before anything in step 6 is attempted.

## 16. Demo sequence

Extends the existing `make demo` shape rather than replacing it.

1. Existing Base fork bring-up, deploy, seed — unchanged, plus `ShareVault` deploy and inventory.
2. Publish the Move package to Sui testnet; write the package ID and pool object ID into the deployment manifest next to the Base addresses.
...
| Vaults and capital allocators | `ShareVault` custody on Base and the Sui `Pool` as the credit ledger; ParityHook's ERC-6909 claim inventory is what makes parity exits possible. |
